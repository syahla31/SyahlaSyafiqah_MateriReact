  ...
  return (
    <div className="min-h-screen flex flex-col justify-center items-center bg-pink-50 p-6">
      <h1 className="text-3xl font-bold text-pink-700 mb-6">
        ✨ Mini Form without React Hook✨
      </h1>

      <form
        onSubmit={handleSubmit}
        className="w-full max-w-lg bg-white p-6 rounded-lg shadow-lg border-2 border-pink-200"
      >
        <div className="mb-4">
          <label
            htmlFor="name"
          ...
          <tbody>
            {tableData.length === 0 ? (
              <tr>
                <td
                  colSpan="3"
                  className="px-6 py-3 text-center text-sm text-pink-500"
                >
              ...
              tableData.map((row, index) => (
                ...
                >
                  <td className="px-6 py-3 text-sm text-pink-600">
                    {row.name}
                  </td>
                  <td className="px-6 py-3 text-sm text-pink-600">
                    {row.email}
